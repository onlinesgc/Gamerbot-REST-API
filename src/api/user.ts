...
user_router.post("/fetch_many", async (req:Request, res:Response) => {
    ...
    createTokenLog(req.headers['authorization'],"POST /api/user/fetch_many",new Date(),{filter:json_body.filter});
...
user_router.post("/:userid", async (req:Request, res:Response) =>{
    ...
    for(let key in json_body){
        if(user_data?.get(key) == null) 
            return res.status(400).json({"error":`no ${key} key was found in the user data`});
...
user_router.post("/create", async (req:Request, res:Response) =>{
    ...
    if(json_body.userID == null ||
       json_body.serverID == null || 
       json_body.lastMessageTimestamp == null || 
       json_body.xpTimeoutUntil == null) return res.status(400).json({"error":"Missing parameters"});

    let new_user_data = await create_user(json_body.userID, json_body.serverID, json_body.lastMessageTimestamp, json_body.xpTimeoutUntil);
