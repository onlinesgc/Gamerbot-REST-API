...
public_frame_router.post("/get/config", async (req: Request, res: Response) => {
    ...
    frame_config.map((frame) => frame.frameLink = `http://localhost:${PORT}/public_api/frame/get/${guild_id}/${frame.id}`);
    return res.json(frame_config);
});

public_frame_router.get("/get/:guildId/:frameId", async (req: Request, res: Response) => {
    const guild_id = sanitize(req.params["guildId"]);
    const frame_id = sanitize(req.params["frameId"]);
    const config = await fetch_guild_config(guild_id);
    //eslint-disable-next-line
    const frame = (config?.frameConfig as unknown as Array<any>)[parseInt(frame_id)];
    if(frame == null) return res.status(400).json({ error: "Invalid frame id" });
